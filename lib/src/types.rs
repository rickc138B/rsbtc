...
impl Blockchain {
    ...
    pub fn add_block(&mut self, block: Block) -> Result<()> {
        ...
        if self.blocks.is_empty() {
            ...
            // blocks's prev_block_hash is the hash of the last block
            let last_block = self.blocks.last().expect("Expected at least one block in the chain");
...
impl Block {
    ...
    pub fn verify_transactions(&self, predicted_block_height: u64, utxos: &HashMap<Hash, TransactionOutput>) -> Result<()> {
        ...
        for transaction in self.transactions.iter().skip(1) {
            ...
            // It is fine for output value to be less than input value // as the difference is the fee for the miner
